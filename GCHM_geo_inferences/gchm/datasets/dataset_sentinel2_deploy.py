...
class Sentinel2Deploy(Dataset):
    ...
    The Sentinel-2 tile is cropped into slightly overlapping patches to apply the model.
    The resulting patches can then be recomposed to the original tile using recompose_patches().

    Args:
        path (str): Path of the .zip file in SAFE format (local path)
    ...
    """
    def __init__(self, path, input_transforms=None, input_lat_lon=True, patch_size=128, border=8):
        ...
        # Use local files for reading Sentinel-2 bands
        self.image, self.tile_info, self.scl, self.cloud = read_sentinel2_bands(data_path=self.path, channels_last=True)
        ...
        # Create lat lon masks for entire images (10m resolution)
        self.lat_mask, self.lon_mask = create_latlon_mask(height=self.ref_ds.RasterYSize, width=self.ref_ds.RasterXSize, refDataset=self.ref_ds)
    ...
    def recompose_patches(self, patches, out_type=np.float32,
        ...
        if mask_tile_boundary:
            # mask the tile boundary pixels
            tile_masked[0:self.border, :] = np.nan
